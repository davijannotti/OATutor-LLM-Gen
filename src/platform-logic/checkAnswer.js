...
function parse(_string) {
    ...
    const string = _string.replace(/\$\$/g, ""); // replace $$ as KAS can parse LaTex but without the $$ prepended/appended
...
function validateAndCorrectFormat(input) {
    ...
    const correctedInputFraction = input.replace(
        fraction,
    ...
    const correctedInput = correctedInputFraction.replace(
        sqrt,
        (match, number) => {
            return `\\sqrt{${number}}`;
        },
...
function checkAnswer({
    ...
    if (variabilization) {
        actual = actual.map((actualAns) =>
            variabilize(actualAns, variabilization),
        );
    }
        ...
        if (answerType === "arithmetic") {
            ...
            if (/\\begin{[a-zA-Z]?matrix}/.test(actual)) {
                    ...
                    return matrix.reduce(
                        (acc, row, idx) =>
                            ...
                            row.reduce((_acc, cell, jdx) => {
                                ...
                                return (
                                    _acc &&
                                    KAS.compare(_studentExpr, _solExpr).equal
                                );
                            }, true),
                        true,
                    );
                ...
                if (IS_STAGING_OR_DEVELOPMENT) {
                    console.debug(
                        "checkAnswer.js: Using KAS to compare answer with solution",
                        "attempt",
                        attempt,
                        "actual",
                        actual,
                    ...
                    console.debug(
                        "checkAnswer.js: questionText vs attempt",
                        questionText,
                        "vs",
                        attempt,
                    );
                ...
                try {
                    ...
                    if (questionTextRepr === parsed.repr()) {
                        return [
                            parsed.print(),
                            false,
                            WrongAnswerReasons.sameAsProblem,
                        ];
                    }
                } catch (_) {
                    // ignored
                ...
                let correctAnswers = _parseEquality(
                    parsed,
                    actual.map((actualAns) => parse(actualAns).expr),
                );
            ...
            parsed = +attempt;
            const correctAnswers = _equality(
                round(parsed, precision),
                actual.map((actualAns) => round(+actualAns, precision)),
            );
